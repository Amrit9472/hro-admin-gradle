...
//		}
		 try {
		        ByteArrayResource pdfResource = generatePdf(savedEmployee, employeeId);
		        emailService.emailsendEmailWithAttachment(employee.getEmail(), "LOI_" + employeeId + ".pdf", pdfResource ,savedEmployee);
		        return pdfResource;
		    } catch (Exception e) {
		        throw new RuntimeException("Failed to generate PDF or send email", e);
		    }
	...
	private ByteArrayResource generatePdf(OurEmployees savedEmployee, Long employeeId) throws IOException {
	    ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();

	    String email = savedEmployee.getEmployee().getEmail();
	    String safeEmail = email.replaceAll("[^a-zA-Z0-9]", "_");
	    String fileName = safeEmail + "_" + employeeId + ".pdf";
	    ...
	    String outputPath = pdfSavePath + File.separator + fileName;
	    try (OutputStream fileOutputStream = new FileOutputStream(outputPath)){
	       
	        OutputStream teeOutputStream = new TeeOutputStream(byteArrayOutputStream, fileOutputStream);

	    	// Use in-memory stream for PDF writer
	        PdfWriter writer = new PdfWriter(teeOutputStream);
	        PdfDocument pdfDocument = new PdfDocument(writer);
	        Document document = new Document(pdfDocument);

	        String name = savedEmployee.getEmployee().getFullName();
	        Long referenceId = employeeId;
	        ...
	        Double loyaltyBonus = savedEmployee.getLoyaltyBonus();
	        Double pli = savedEmployee.getPli();
	        Double ctc = savedEmployee.getCtc();
//	        String email = savedEmployee.getEmployee().getEmail();

	        // Fonts
	        PdfFont helvetica = PdfFontFactory.createFont(StandardFonts.HELVETICA);
	        PdfFont timesNewRomanFont = PdfFontFactory.createFont("src/main/resources/fonts/Times-New-Roman.ttf", PdfFontFactory.EmbeddingStrategy.PREFER_EMBEDDED);
	        PdfFont timesNewRomanFontBold = PdfFontFactory.createFont("src/main/resources/fonts/Times-New-Roman-Bold.ttf", PdfFontFactory.EmbeddingStrategy.PREFER_EMBEDDED);
	        PdfFont timesNewRomanFontItalic = PdfFontFactory.createFont("src/main/resources/fonts/Times-New-Roman-Italic.ttf", PdfFontFactory.EmbeddingStrategy.PREFER_EMBEDDED);
	        PdfFont arial = PdfFontFactory.createFont("src/main/resources/fonts/ARIAL.ttf", PdfFontFactory.EmbeddingStrategy.PREFER_EMBEDDED);
	        ...
	        // Greeting
	        document.add(new Paragraph().add(new Text("Dear ").setFont(arial))
	        ...
	        document.add(new Paragraph("(Detailed salary annexure would be shared along with your appointment letter subject to all relevant tax laws)")
	                .setFontSize(10).setMarginTop(0));

	        document.add(new Paragraph("We look forward for a long-lasting performance and growth oriented association with you. "
	        ...
	        if ("Yes".equalsIgnoreCase(savedEmployee.getTrainingApplicable())) {
	            ...
	            document.add(new Paragraph("The training duration for the process would be of " + trainingDays + " days and the stipend will be credited along with your 2nd month salary. "
	            ...
	            document.add(new Paragraph("Training period can extend by 3-4 working days depending upon the content coverage, and trainees capability in learning the subject matter. "
	...
	/**private ByteArrayResource generatePdf(OurEmployees savedEmployee ,Long employeeId) throws java.io.IOException {
		ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
		try {
//	        String outputPath = "C:/Users/Bot_25/Desktop/Documents/AadharFileUpload/LOI_" + employeeId + ".pdf";
			  
			String email = savedEmployee.getEmployee().getEmail(); // Get email from related Employee object
			String safeEmail = email.replaceAll("[^a-zA-Z0-9]", "_"); // Replace non-alphanumeric characters
...
//	        String outputPath = pdfSavePath + "/LOI_" + employeeId + ".pdf";
	        String outputPath = pdfSavePath + safeEmail + "_" + employeeId + ".pdf";
